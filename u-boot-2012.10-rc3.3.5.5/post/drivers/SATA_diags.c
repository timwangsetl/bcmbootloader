...
#if 0
    printf("Read one more sector for checking...");
    if (result) {
        /* FIXME: as POST, any nonzero fails; as single, must be -1 to fail */
        result = -1;
    }
    for (port = 0; port < maxPorts; port++) {
        /* one more read so the driver is in sync with itself */
        AhciFillCmdSlot(port,
                        0 /* tag */,
                        false /* write */,
                        true /* DMA */,
        ...
        AhciFilSg(port,
                  ...
                  false /* blank */);
        AhciIssueCmd(port, 0 /* tag */);
        wait_command_done(port, 0 /* tag */, true /* DMA */, 1);
        printf(" %d",  port);
    }
#endif
error:
...
sata_post_test_read(int flags)
{
    ...
    /* FIXME: assume 512 bytes per sector */
    post_log("test: |");
...
static int run_pcie_sata(void)
{
    ...
    for ( j = 0; j <=1000; j+=4 )
    {
        if ( 0 == pcie_SATA_DMA_RD_WR_verify(0, 0, 4, 1000))
        {
                printf(" . ");
        }
        else
