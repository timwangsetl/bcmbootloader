...
static u32 mspi_translation(u32 tx_data, u32 tx_len, u32 rx_len)
{
    ...
    *(unsigned int *)QSPI_MSPI_ENDQP = total - 1;
    ...
    // Start it
    *(unsigned int *)QSPI_MSPI_STATUS = 0x00;
    *(unsigned int *)QSPI_MSPI_SCPR2 = 0xc0;
...
void qspi_init(void)
{
    ...
    if (*(unsigned int *)QSPI_BSPI_MAST_N_BOOT_CTRL == 0) {
        // Wait for BSPI ready
        //post_log("\nWait for BSPI ready");
        ...
        // Switch to MSPI
        //post_log("\nSwitch to MSPI");
        *(unsigned int *)QSPI_BSPI_MAST_N_BOOT_CTRL = 1;
    }

    // Basic MSPI configuration
    *(unsigned int *)QSPI_MSPI_SPCR0_LSB = 0x40;    //baud rate = system_clcok/(2 * 62)
    *(unsigned int *)QSPI_MSPI_SPCR0_MSB = 0xa3;    //SPI master, no waiting before transaction, SCK=1.
    ...
    for(i = 0; i < 4096/8; i++) {
	   ...
	   int j;
       for ( j = 0; j < 8; j++) {
          *(pu8_4KBytes+j + i*8) = (unsigned char) *(u32 *)(rxaddr + j* 8);
          printf("d%x ", *(pu8_4KBytes+j + i*8) );        
       }
       u32FlashAddress = u32FlashAddress + 8;
           printf(" %x\n",  u32FlashAddress);
#endif
    }
...
int qspi_post_test (int flags)
{
    ...
    //----------------------------------------------------------
    *(unsigned int *)QSPI_BSPI_MAST_N_BOOT_CTRL = 0;     //SPI bus is driven by BSPI

    // Invalidate bspi buffer
    *(unsigned int *)QSPI_BSPI_B0_CTRL = 1;     //clear data in buffer 0
    *(unsigned int *)QSPI_BSPI_B1_CTRL = 1;     //clear data in buffer 1
    ...
    *(unsigned int *)QSPI_BSPI_STRAP_OVERRIDE_CTRL = 3;       //3-byte address, dual mode, override strap
    *(unsigned int *)QSPI_BSPI_FLEX_MODE_ENABLE    = 1;       //enable flex mode
    *(unsigned int *)QSPI_BSPI_BITS_PER_CYCLE      = 0x1010101; //dual mode to recv data, 2 mode bits/cycle, 2 address bits/cycle, 2 cmd bits/cycle
    *(unsigned int *)QSPI_BSPI_BITS_PER_PHASE      = 0x8;     //dummy cycle=8, 0 mode bits, 24-byte address
    *(unsigned int *)QSPI_BSPI_CMD_AND_MODE_BYTE   = 0x3;     //default command is fast read
    ...
    *(unsigned int *)QSPI_BSPI_STRAP_OVERRIDE_CTRL  = 9;  //3-byte address, quad mode, override strap
    *(unsigned int *)QSPI_BSPI_FLEX_MODE_ENABLE     = 1;  //enable flex mode
    *(unsigned int *)QSPI_BSPI_BITS_PER_CYCLE       = 0x2020202; //quad mode to recv data, 4 mode bits/cycle, 4 address bits/cycle, 4 cmd bits/cycle
    *(unsigned int *)QSPI_BSPI_BITS_PER_PHASE       = 8;    //dummy cycle=8, 0 mode bits, 24-byte address
    *(unsigned int *)QSPI_BSPI_CMD_AND_MODE_BYTE    = 0x3;     //default command is fast read

    // Invalidate bspi buffer
    *(unsigned int *)QSPI_BSPI_B0_CTRL = 1;
    *(unsigned int *)QSPI_BSPI_B1_CTRL = 1;
