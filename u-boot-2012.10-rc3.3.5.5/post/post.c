...
int post_info(char *name)
{
    ...
    if ( post_get_board_diags_type() == NO_DIAGS_SUPPORT )
    {
        printf("Supported boards:BCM958300K/BCM958302K/BCM911360K\n");
        printf("Set DIAGS_BOARD. saveenv and then power cycle to change the env\n");
    }
#endif
	if (name == NULL) {
    ...
    do
    {
        ...
        if(nbytes == 1)
        {
        	if ((console_buffer[0] >= '0') && (console_buffer[0] <= '9'))
        		return (console_buffer[0]-'0');            
        } else {
            if ((console_buffer[0] >= '0') && (console_buffer[0] <= '9') 
           	    && (console_buffer[1] >= '0') && (console_buffer[1] <= '9'))
           	    return (console_buffer[0]-'0')* 10 + (console_buffer[1]-'0');     
        }
    }while ( 1 );
}
...
int post_check_board_cfg_env( void )
{
    ...
    if (strcmp(var, "BCM958300K")==0)
    {
        gDiags_board = BCM958300K;
        puts("Found BCM958300K diag support\n");
    }
    else if (strcmp(var, "BCM958302K")==0)
    {
        gDiags_board = BCM958302K;
        puts("Found BCM958302K diag support\n");
    }
    else if (strcmp(var, "BCM911360K")==0)
    {
        gDiags_board = BCM911360K;
        puts("Found BCM911360K diag support\n");
    }
    else
    {
        gDiags_board = NO_DIAGS_SUPPORT;
