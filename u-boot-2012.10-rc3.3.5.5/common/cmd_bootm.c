...
//		printf("   Uncompressing %s ... ", type_name);
	...
	printf("WARNING-Cygnus:%d:%s() fast boot fix, disabled flush_cache()\n", __LINE__, __func__);
#else
...
#if defined(CONFIG_CYGNUS_EMULATION) || defined(CONFIG_IPROC_EMULATION)
		printf("\nWARNING-Cygnus/Emulator:%d:%s() disabled checksum verification \n", __LINE__, __func__);
#else
