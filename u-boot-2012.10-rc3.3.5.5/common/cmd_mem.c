...
int do_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (argc > 1)
		start = (vu_long *)simple_strtoul(argv[1], NULL, 16);
	else
		start = (vu_long *)CONFIG_SYS_MEMTEST_START;

	if (argc > 2)
		end = (vu_long *)simple_strtoul(argv[2], NULL, 16);
	else
		end = (vu_long *)CONFIG_SYS_MEMTEST_END;
...
int do_wd_reboot ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	*(unsigned int *)(0x1800c180) = 0xea68;                             // unlock
	*(unsigned int *)(0x1800c15c) = 0x0003061c;     // bit[7:4]: reset indicator; set bit 4 to 1
	*(unsigned int *)(0x1800c154) = 0x83c00000;     // write bit31=1 , sel_SW_setting
 
	*(unsigned int *)(0x18000080) = 0x1;   // set watchdog count register to 1
	while (1);  //reboot here
