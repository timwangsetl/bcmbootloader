...
int usb_new_device(struct usb_device *dev)
{
	 ...
	 * invalid header while reading 8 bytes as device descriptor. */
		...
		err = usb_get_descriptor(dev, USB_DT_DEVICE, 0, tmpbuf, USB_DT_DEVICE_SIZE);
		if (err < USB_DT_DEVICE_SIZE) {
			printf("\n      USB device not responding, " \
				   "giving up (status=%lX)\n", dev->status);
			return 1;
		}

		memcpy(&dev->descriptor, tmpbuf, USB_DT_DEVICE_SIZE);
	#else
	...
	mdelay(30);	/* Let the SET_ADDRESS settle */
