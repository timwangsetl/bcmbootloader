...
#define MAX_SPI_BYTES 1024	/* Maximum number of bytes we can handle */
#else
...
int do_spi (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if (bus > 0){
        ...
        j = pl022_spi_xfer(slave, bitlen, dout, din, SPI_XFER_BEGIN | SPI_XFER_END);
    }else {
#endif
        spi_claim_bus(slave);
        j = spi_xfer(slave, bitlen, dout, din, SPI_XFER_BEGIN | SPI_XFER_END);
#if defined(CONFIG_CYGNUS)
    }
#endif
