...
#define BOOT_SIZE	0x00100000
#define KERNEL_FS_SIZE 0x00e80000
...
int do_copy(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	int rc, ret;
	char *address = "0x61000000";
	char p[50];
	char *local_argv[3];
	char *local_argv1[5];
	
    //check style
    if(strncmp(argv[1], "tftp", strlen(argv[1])))
    {
    	printf("Unknown command\n");
    	return 0;
    }
    
    if(strncmp(argv[4], "flash", strlen(argv[4])))
    {
    	printf("Unknown command\n");
    	return 0;
    }
    ...
    if(argc == 6)
    {
    	if(strncmp(argv[5], "img", strlen(argv[5]))
    	    &&strncmp(argv[5], "uboot", strlen(argv[5])))
    	{
    		printf("Unknown command\n");
    		return 0;
    	}
    ...
    //check style over	
	strncpy(p, argv[3], strlen(argv[3]));
	strncpy(p + strlen(argv[3]), ":", 1);
	strcpy(p + strlen(argv[3])+1,  argv[2]);

	local_argv[0] = cmdtp->name;
	local_argv[1] = address;
	local_argv[2] = p;

	rc = netboot_common (TFTPGET, cmdtp, 3, local_argv);
	if(rc) 
		return 0;
    ...
    if(argc == 6)
    {
        ...
        if(strncmp(argv[5], "uboot", strlen(argv[5])) == 0)
        {
            ret = spi_flash_erase(flash, 0, 0x80000);
            if (ret) {
            ...
            /* Write data */
            ret = spi_flash_write(flash, 0, 0x80000, 0x61000000);
            if (ret) {
        ...
        if(strncmp(argv[5],"img", strlen(argv[5])) == 0)
        {
            ...
            /* Write data */
            ret = spi_flash_write(flash, 0, 0x1000000, 0x61000000);
            if (ret) {
                printf("SPI flash write failed, error=%d\n", ret);
            }
    ...
    {
        ret = spi_flash_erase(flash, 0x100000, 0x0e80000);
        if (ret) {
            printf("SPI flash erase failed, error=%d\n", ret);
            spi_flash_free(flash);
            return ret;
        ...
        /* Write data */
        ret = spi_flash_write(flash, 0x100000, 0x0e80000, 0x61100000);
        if (ret) {
            printf("SPI flash write failed, error=%d\n", ret);
        }
...
int do_ip(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	//ip address
	if(strncmp(argv[1], "address", strlen(argv[1])) == 0) {
		setenv("ipaddr", argv[2]);
		setenv("netmask", argv[3]);
		return 1;		
	} 
	//ip mac
		else if(strncmp(argv[1], "mac", strlen(argv[1])) == 0) {
		setenv("ethaddr", argv[2]);
		return 1;
	}
		
		else if(strncmp(argv[1], "default-getway", strlen(argv[1])) == 0){
		setenv("getwayip", argv[2]);
		return 1;		
	} 
...
int do_delete(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int ret;
    struct spi_flash *flash;		
    ...
    ret = spi_flash_erase(flash, 0x1f80000, 0x40000);
    if (ret) {
        printf("SPI flash erase failed, error=%d\n", ret);
        spi_flash_free(flash);
        return ret;
    ...
    spi_flash_free(flash);
	printf("\nRestore default settings ok, please reboot!\n");    
...
U_BOOT_CMD(
	ip, 4, 1,	do_ip,
	"Configure the the IPaddress of the the Ethernet",
	"address     -- Set device ip address\n"
);

U_BOOT_CMD(                                            
	delete,	1,	1,	do_delete,
