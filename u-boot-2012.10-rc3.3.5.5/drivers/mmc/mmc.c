...
#if defined(BCMHANA_MMC_FPGA_NO_HIGH_SPEED)
	// Don't send High speed switch command for bcmhana in case of working on FPGA.
...
int mmc_startup(struct mmc *mmc)
{
	...
	// Have some delay for bcmhana FPGA.
	udelay(100000) ;
...
int mmc_init(struct mmc *mmc)
{
	...
	//printf("\nError after mmc_send_if_cond() ==> %d \n", err);
	/* Now try to get the SD card's operating condition */
	err = sd_send_op_cond(mmc);

	//printf("\nError after sd_send_op_cond()==> %d \n", err);
