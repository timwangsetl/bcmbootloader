...
//    printf("\nInit port to gen %d\n",SPEED_GEN3);
        return (ahci_init_one(SPEED_NO_LIMIT));
...
void AHCI_DMA_Write(int port, int tag, int blk, u32 lba)
{
        ...
        AhciPrepCmd(port,tag,WR,DMA,blk,lba,RST_OFF, NULL, false);
...
void AHCI_DMA_read(int port, int tag, int blk, u32 lba, int blank)
{

        AhciPrepCmd(port,tag,RD,DMA,blk,lba,RST_OFF, NULL, blank);
        /* debug("Doing:  AhciIssueCmd\n"); */
        AhciIssueCmd(port, tag);
...
int wait_command_done(int port, int tag, int pio, int blk)
{
...
#if WAIT_WITH_MEMCPY
        timeout = 60 * 1000; /* rather more than a minute */
#else /* WAIT_WITH_MEMCPY */
        timeout = 60 * 1000000; /* a bit more than a minute */
#endif /* WAIT_WITH_MEMCPY */
        ...
        while (*pTmp32 & msk) {
#if WAIT_WITH_MEMCPY
            memcpy(0x61000000, 0x62000000, 0x00100000);
            flush_dcache_range(0x61000000, 0x61100000);
            invalidate_dcache_range(0x61000000, 0x61100000);
            invalidate_dcache_range(0x62000000, 0x62100000);
#else /* WAIT_WITH_MEMCPY */
            udelay(1);
#endif /* WAIT_WITH_MEMCPY */
            timeout--;
            if (!timeout) {
                debug("Device did not respond\n");
                return -1;
            }
        }
...
int check_command_done(int port, int tag, int pio, int blk)
{
        u32 s1BaseAddr, s2BaseAddr, msk, rxFis, fisType;
        volatile u32 *pTmp32;
...
/* WARNING: assumes command has completed (see check_command_done) */
int
check_command_result(int port, int tag, int pio, int blk)
{
    u32 baseAddr;
    u32 value;
    u8 taskStatus;
    ...
    baseAddr = SATA_AHCI_PORT0_S1 + (PORT_OFFSET * port);

    ptr = (u32*)(baseAddr + PxTFD);
    value = *ptr;
    taskStatus = value & 0xFF;
    taskError = (value >> 8) & 0xFF;
    if (taskStatus & 0x01) {
        /* error bit is set */
        result = -1;
        debug("error bit is set; command resulted in an error\n");
    }
    ptr = (u32*)(baseAddr + PxSERR);
    value = *ptr;
    if (value) {
        result = -1;
        debug("PxSERR has bits set; something went wrong\n");
    }
    if (result) {
        debug("Task file status %02X, error %02X; PxSERR %08X\n",
              taskStatus,
              taskError,
              value);
    }
    /* FIXME: check sectors transferred? */
    return result;
}
...
int AhciFilSg(int port, int tag, u32 addr, int byteCount, int bytesPerTransfer, int blank)
{
        u32 *prd_addr;
        u32 prdtl;
        u32 cmdtbl_base;
        ...
        if (bufAddr & 0x1) {
            debug("buffer must start on even byte boundary");
            return -1;
        }
        if (byteCount & 0x1) {
            debug("buffer length must be an even number of bytes");
            return -1;
        }
        if (bytesPerTransfer & 1) {
            debug("bytes per PRD must always be an even number");
            return -1;
        }
        if ((bytesPerTransfer < 2) || (bytesPerTransfer > XFER_PER_PRD)) {
            bytesPerTransfer = XFER_PER_PRD;
        }
        prdtl = (byteCount + (bytesPerTransfer - 1)) / bytesPerTransfer;
        ...
        /* not running virtual, so contiguous memory looks contiguous */
        size = byteCount;
        prd_cnt = 0;
        do /* while ((size > 0) && (prd_cnt < PRD_NUM_MAX)) */ {
            *prd_addr = bufAddr /* low 32b of address */;
            prd_addr++;
            *prd_addr = 0 /* high 32b of address */;
            prd_addr++;
            *prd_addr = 0 /* reserved */;
            prd_addr++;
            if (size > bytesPerTransfer) {
                /* split off first part of transfer to a PRD */
                /* leave 'I' (bit 31) clear */
                *prd_addr = (bytesPerTransfer - 1);
                /* adjust for next part of buffer */
                bufAddr += bytesPerTransfer;
                size -= bytesPerTransfer;
            } else {
                /* can fit the remaining transfer in this PRD */
                *prd_addr = size - 1;
                bufAddr += size;
                size = 0;
            }
            prd_addr++;
            prd_cnt++;
        } while ((size > 0) && (prd_cnt < prdtl));
        if (size) {
            debug("transfer %u is too long to fit in %u PRDs",
                  byteCount,
                  PRD_NUM_MAX);
            return -1;
        }
        ...
        *pTmp32 = (*pTmp32 & 0x0000FFFF) | (prd_cnt << 16);
        ...
        /* ensure command buffers and S/G table written to DRAM */
        flush_dcache_range(cmdtbl_base,
                           cmdtbl_base + CMD_TBL_SIZE + (PRD_ENTRY_SIZE * PRD_NUM_MAX));

        /* init buffer */
        if (blank) {
            memset((void*)addr, 0x00, byteCount);
            flush_dcache_range(addr, addr + byteCount);
        }
#if 0
        prd_addr = (u32 *)(cmdtbl_base + 0x80);
...
void AhciPrepCmd(int port,int tag,int w,int pio,int blk,u32 lba,int rst, void* buffer, int blank)
{
  ...
  if (!buffer) {
      /* use default buffer */
      if (w == WR) {
          AhciFilSg(port, tag, WR_BUF + (tag * (512 * blk)), (512 * blk), 0, false);
      } else {
          AhciFilSg(port, tag, RD_BUF + (tag * MAX_BUF_SG_SIZE), (512 * blk), 0, blank);
      }
  } else {
      /* caller provided a buffer */
      AhciFilSg(port, tag, (u32)buffer, 512 * blk, 0, blank);
  }
}
...
int SpinUpAndWait (int port, int speed)
{
        ...
        printf("\nSCTL = %08X, SSTS = %08X, SERR = %08X\n",
               readlx(s1BaseAddr + PxSCTL),
               readlx(s1BaseAddr + PxSSTS),
               readlx(s1BaseAddr + PxSERR));
...
int  iproc_SATA_DMA_RD_WR_verify(int port, int tag, int blk, u32 lba)
{
        ...
        AhciPrepCmd(port,tag,RD,DMA,blk,lba,RST_OFF, NULL, true);
        /*debug("Doing:  AhciIssueCmd\n"); */
        AhciIssueCmd(port, tag);
        /*debug("Doing:  wait_command_done\n");   */
        wait_command_done (port, tag, DMA, blk);
...
int CmpBuf(int port, int tag, int blkCnt)
{
        ...
        for (i = 0; i < cnt; i++)
        {
                if (*pWr != *pRd)
                {
                        printf("** ERROR: Data Miscompared at word offset %lx***\n",cnt);
                        printf("Expect: 0x%lx Read: 0x%lx\n",*pWr, *pRd);
                        return 1;
                }
