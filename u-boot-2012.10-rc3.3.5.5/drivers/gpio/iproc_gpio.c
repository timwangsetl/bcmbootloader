...

{
    ...
    {   
        if (tmp & ( 1 << iter ))
        {
	        ...
	        /* clear the interrupt*/
	        *(volatile unsigned long*)(ChipcommonG_GP_INT_CLR) |= (1<<iter);
	        IntReceived = 1;
	    }
    }
...
void iprocGPIOInit( void )
{
    ...
    gpioDev.ioaddr =(void*) ChipcommonG_GP_DATA_IN;//(void*)IPROC_GPIO_CCA_BASE;
    gpioDev.dmu_ioaddr = 0;//(void*)NORTHSTAR_DMU_BASE;
    gpioDev.intr_ioaddr =(void*) ChipcommonG_GP_DATA_IN;//(void*)CCA_CHIPID ;
    gpioDev.irq_base = 0;
#else
