    ...
    if (CONFIG_IPROC_BSPI_DATA_LANES != 1 || 
        ...
        if (CONFIG_IPROC_BSPI_DATA_LANES == 4) {
            priv->bspi_hw->bits_per_cycle |= 
                cpu_to_le32(2 << BSPI_BITS_PER_CYCLE_DATA_SHIFT);
        } else {
            priv->bspi_hw->bits_per_cycle |= 
                cpu_to_le32((CONFIG_IPROC_BSPI_DATA_LANES - 1) 
                    << BSPI_BITS_PER_CYCLE_DATA_SHIFT);
        }
        if (CONFIG_IPROC_BSPI_ADDR_LANES == 4) {
            priv->bspi_hw->bits_per_cycle |= 
                cpu_to_le32(2 << BSPI_BITS_PER_CYCLE_ADDR_SHIFT);
        } else {
            priv->bspi_hw->bits_per_cycle |= 
                cpu_to_le32((CONFIG_IPROC_BSPI_ADDR_LANES - 1) 
                    << BSPI_BITS_PER_CYCLE_ADDR_SHIFT);
        }
        
        /* Dummy cycles */
        priv->bspi_hw->bits_per_phase &= cpu_to_le32(~0xFF);
        priv->bspi_hw->bits_per_phase |= cpu_to_le32(CONFIG_IPROC_BSPI_READ_DUMMY_CYCLES);
        
        /* Command byte for BSPI */
        priv->bspi_hw->cmd_and_mode_byte = cpu_to_le32(CONFIG_IPROC_BSPI_READ_CMD);
        
        /* Enable flex mode to take effect */
        priv->bspi_hw->flex_mode_enable = cpu_to_le32(1);
    }
...
bcmspi_disable_bspi(struct bcmspi_priv *priv)
{
    ...
    if ((priv->bspi_hw->mast_n_boot_ctrl & cpu_to_le32(1)) == 0) {
        ...
        if (priv->bspi_hw->mast_n_boot_ctrl != cpu_to_le32(1)) {
            return -1;
        }
    }
...
bspi_set_4byte_mode(struct bcmspi_priv *priv, int enable)
{
    ...
    if (enable) {
        ...
        priv->bspi_hw->bits_per_phase |= cpu_to_le32(BSPI_BITS_PER_PHASE_ADDR_MARK);
...
bspi_read_via_raf(struct bcmspi_priv *priv, u8 *rx, uint bytes)
{
    ...
    for(;;) {
        ...
        if (!(status & cpu_to_le32(BSPI_RAF_STATUS_FIFO_EMPTY_MASK))) {
            (void)priv->bspi_hw_raf->read_data;
            continue;
        }
        if (!(status & cpu_to_le32(BSPI_RAF_STATUS_SESSION_BUSY_MASK))) {
            break;
        }
    }
    ...
    /* Setup hardware */
    if (priv->mode_4byte) {
        u32 val = cpu_to_le32(priv->bspi_addr & 0xFF000000);
        if (val != priv->bspi_hw->flash_upper_addr_byte) {
            priv->bspi_hw->flash_upper_addr_byte = val;
            bspi_flush_prefetch_buffers(priv);
        }
    }
    priv->bspi_hw_raf->start_address = cpu_to_le32(priv->bspi_addr & 0x00FFFFFF);
    priv->bspi_hw_raf->num_words = cpu_to_le32(words);
    priv->bspi_hw_raf->watermark = 0;
    
    /* Kick off */
    priv->bspi_hw_raf->ctrl = cpu_to_le32(BSPI_RAF_CONTROL_START_MASK);
    ...
    while(bytes) {
        status = priv->bspi_hw_raf->status;
        if (!(status & cpu_to_le32(BSPI_RAF_STATUS_FIFO_EMPTY_MASK))) {
		u32 data = priv->bspi_hw_raf->read_data;
...
mspi_xfer(struct bcmspi_priv *priv, uint bytes, const u8 *tx, u8 *rx, int end)
{
    if (bytes & 1) {
        ...
        if (priv->mspi_16bit) {
            priv->mspi_hw->spcr0_msb |= cpu_to_le32(MSPI_SPCR0_MSB_BITS_8);
            priv->mspi_16bit = 0;
        }
    } else {
    ...
    while(bytes) {
        ...
        if (priv->mspi_16bit) {
            ...
            /* Fill CDRAMs */
            for(i=0; i<queues; i++) {
                priv->mspi_hw->cdram[i] = cpu_to_le32(0xc2);
            }
            
            /* Fill TXRAMs */
            for(i=0; i<chunk; i++) {
                priv->mspi_hw->txram[i] = cpu_to_le32(tx? tx[i] : 0xff);
            }
            ...
            for(i=0; i<chunk; i++) {
                priv->mspi_hw->cdram[i] = cpu_to_le32(0x82);
                priv->mspi_hw->txram[i << 1] = cpu_to_le32(tx? tx[i] : 0xff);
            }
        }
        
        /* Setup queue pointers */
        priv->mspi_hw->newqp = 0;
        priv->mspi_hw->endqp = cpu_to_le32(queues - 1);

        /* Deassert CS if requested and it's the last transfer */
        if (bytes == 0 && end ) {
            priv->mspi_hw->cdram[queues - 1] &= cpu_to_le32(~0x80);
        }
        
        /* Kick off */
        priv->mspi_hw->mspi_status = 0;
        priv->mspi_hw->spcr2 = cpu_to_le32(0xc0);    /* cont | spe */
        ...
        if ((priv->mspi_hw->mspi_status & cpu_to_le32(1)) == 0) {
            return -1;
        }
        ...
        if (rx) {
            if (priv->mspi_16bit) {
                for(i=0; i<chunk; i++) {
                    rx[i] = 
                        le32_to_cpu(priv->mspi_hw->rxram[i]) & 0xff;
                }
            } else {
                for(i=0; i<chunk; i++) {
                    rx[i] = 
                       le32_to_cpu(priv->mspi_hw->rxram[(i << 1) + 1]) & 0xff;
                }
            }
        }
