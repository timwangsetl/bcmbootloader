...
extern u32	com2Configured;
extern u32 	com3Configured;
extern int serial_init_nsp(void);
#endif
...
/* Function to relocate data and bss sections to RAM area */
void relocate_data()
{

	 extern char __data_start_load, __data_start, __bss_start, _end;
     char *src = &__data_start_load;
     char *dst = &__data_start;

     /* ROM/FLASH has data at end of text; copy it. */
     while (dst < &__bss_start) {
       *dst++ = *src++;
     }
...
{
#ifndef	CONFIG_SWANG_DEBUG_BUILD
	bd_t *bd;
...
#ifndef CONFIG_L2C_AS_RAM /* TLB table is pre-defined for L2C as RAM */
	 ...
	 */
	addr -= gd->mon_len;
	addr &= ~(4096 * 4 - 1); /* MMU on flash fix: 16KB aligned */
...
//	display_dram_config();	/* and display it */
	...
	addr_sp = 0x2000000;  // high 32Mbyte zone
...
#ifdef CONFIG_POST
	/* swang disable the power test for now
	post_bootmode_init();
	post_run (NULL, POST_ROM | post_bootmode_get(0));
	*/
	...
	gd->relocaddr = 0x0;//swang, no relocation
	gd->start_addr_sp = addr_sp;
	gd->reloc_off = addr - _TEXT_BASE;
	printf("relocation Offset is: %08lx\n", gd->reloc_off);
	memcpy(id, (void *)gd, sizeof(gd_t));
	printf("relocate_code: sp = %x, id = %x, addr = %x\n", addr_sp, id, 0);
#ifndef CONFIG_NO_CODE_RELOC
	relocate_code(addr_sp, id, 0);
...
#else
	malloc_start = 0x3000000;   // high 48Mbyte zone
#endif
...
/* check the board types for NSP only */
	...
	if ( com2Configured )
	{
		puts ("COM2 configured\n");
	}
	else
	{
		puts ("COM2 is not configured due to board type\n");
	}
	if ( com3Configured )
	{
		puts ("COM3 configured\n");
	}
	else
	{
		puts ("COM3 is not configured due to board type\n");
	}
#endif
#endif
#endif
