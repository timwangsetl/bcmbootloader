...
unsigned int verify_mem_block_mips( ddr40_addr_t memc_reg_base, ddr40_addr_t addr, unsigned int size , unsigned int mt_mode, int wl )
{
    uint32_t result = 0;
    ...
    // CleanMemory( SrcAddress , 2 * nLengthInDwords , 0xAABBCCDD );
...
unsigned int mtest_verify( ddr40_addr_t memc_reg_base , ddr40_addr_t addr , unsigned int * data_expected, 
                          unsigned long mt_mode, unsigned long wrd_ln )
{
    unsigned int nRet = 0;
    uint32_t const nLengthInDwords = 128 /* addr */;
...
void InitMemory( ddr40_addr_t Address , unsigned long const nLengthInDwords , unsigned long const Seed )
{
    ...
    // Handle to the function should be the same in order to get the same reproducible pseudo-random values.
    ...
    memcpy( Address , aData , DATA_TRANSFER_LENGTH_BYTES );
...
unsigned int CheckMemory( ddr40_addr_t Address , unsigned long const nLengthDwords , unsigned int const Seed , unsigned int wrd_ln )
{
    uint32_t i;
    uint32_t aData[ DATA_TRANSFER_LENGTH_DWORDS + 1 ];
    ...
    memcpy( aData , Address , DATA_TRANSFER_LENGTH_BYTES );
...
void CleanMemory( unsigned long Address , unsigned long const nLengthInDwords , unsigned int const nMask )
{
    uint32_t i;
...
unsigned int CheckMemoryV2( unsigned long Address , unsigned long const nLengthDwords , unsigned int wrd_ln )
{
    uint32_t i;
    uint32_t result_dst = 0;
    uint32_t Value = 0xAAAAAAAAul;
...
// For bitflip test
unsigned int CheckMemoryV3( unsigned long Address , unsigned long const nLengthDwords , unsigned int wrd_ln , int const j )
{
    uint32_t i;
    uint32_t result_dst = 0;
    uint32_t pattern = 0x80000000ul >> j;
...
static inline void generic_calib_steps( ddr40_addr_t phy_reg_base )
{
    ...
    WRREG( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIBRATE , DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIBRATE_calib_steps_MASK ); // calib_steps = 1

    while( true ) {
        uint32_t const regval = RDREG( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS );

        if ( (regval & DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS_calib_idle_MASK) == DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS_calib_idle_MASK ) {
            break;
        }
    }

    NL();
    UART_OUT('S'); plot_hex_number( tb_r(phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS + GLOBAL_REG_RBUS_START) );
    NL();
...
static inline void generic_shmoo_cleanup ( unsigned long phy_reg_base , unsigned int wl_in_use )
{
    ...
    for ( wl = 0 ; wl < wl_in_use ; wl++ ) {
        WRREG( (phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + wl * PHY_REG_OFFSET_PER_WL) , 0x1 );
    }
...
int do_shmoo( ddr40_addr_t phy_reg_base ,
              ddr40_addr_t memc_reg_base ,
              unsigned int option ,
              ddr40_addr_t mem_test_base )
{
    ...
    // printk( KERN_DEBUG "%s: Finish shmoo, return values is 0x%X\n" , __FUNCTION__ , nRet );

    return nRet;
}

static unsigned one_shmoo_RE_FIFO(unsigned int  * word_new_step, 
                                  unsigned int  * byte_new_step, 
                                  unsigned int    option, 
                                  ddr40_addr_t   phy_reg_base,
                                  ddr40_addr_t   memc_reg_base,
                                  ddr40_addr_t   mem_test_base );
                                   ...
                                   ddr40_addr_t   mem_test_base )
{
    ...
    ticks_to_add = ( GET_FIELD( tb_r( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS + GLOBAL_REG_RBUS_START ) ,
                                      DDR40_CORE_PHY_CONTROL_REGS , VDL_CALIB_STATUS , calib_total ) ) / 4;
    
    for ( wl = 0 ; wl < (((option >> 8) & 0xFF) / BITS_PER_WL) ; wl++ ) {
        memsys_reg_offset = phy_reg_base + GLOBAL_REG_RBUS_START + wl * PHY_REG_OFFSET_PER_WL;
        ...
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_step , word_setting );
...
#endif
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , ovr_step , byte_setting );

        tb_w( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + memsys_reg_offset , data );
        tb_w( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + memsys_reg_offset , data );
        ...
        while ( passing == 0 ) {
            ...
            fifo_result[ 0 ] = GET_FIELD( tb_r( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + memsys_reg_offset ) ,
                                          DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status0 );
            fifo_result[ 1 ] = GET_FIELD( tb_r( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + memsys_reg_offset ) ,
                                          DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status1 );
            // Clear the FIFO status for this WL
            tb_w( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + memsys_reg_offset , DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR_clear_MASK );
        ...
        wlx_start[ wl ] = GET_FIELD( tb_r( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + memsys_reg_offset ) ,
                                     DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_step );

        //  if (wl0_bl0_pass == 1) {
        if ( wlx_blx_pass[ wl * RDEN_WORD_STEPS ] == 1 ) {
            wlx_blx_start[ wl * RDEN_WORD_STEPS ] = GET_FIELD( tb_r( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + memsys_reg_offset ) ,
            ...
            // (1, or 3)
            wlx_blx_start[ wl * RDEN_WORD_STEPS + 1 ] = GET_FIELD( tb_r( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + memsys_reg_offset ) ,
        ...
        if ( word_new_step[ wl ] > 63 ) {       
            ...
            word_new_step[ wl ] = 63;
            byte_new_step[ wl * RDEN_WORD_STEPS ]     = wlx_blx_start[ wl * RDEN_WORD_STEPS ] + common;
            byte_new_step[ wl * RDEN_WORD_STEPS + 1 ] = wlx_blx_start[ wl * RDEN_WORD_STEPS + 1 ] + common;
        }    
        else {
            byte_new_step[ wl * RDEN_WORD_STEPS ]     = wlx_blx_start[ wl * RDEN_WORD_STEPS ];
            byte_new_step[ wl * RDEN_WORD_STEPS + 1 ] = wlx_blx_start[ wl * RDEN_WORD_STEPS + 1 ];
        }   
...
unsigned int all_shmoo_RE_byte_FIFO( unsigned long phy_reg_base ,
                                     ...
                                     unsigned long mem_test_base )
{
    ...
    unsigned int  data;
    //
    // Can be moved to the first for loop
    //
    // unsigned int  word_step[ WORD_STEP_COUNT ];
    // unsigned int  byte_step[ BYTE_STEP_COUNT ];
    unsigned int  word_new_step[ WORD_STEP_COUNT ];
    unsigned int  byte_new_step[ BYTE_STEP_COUNT ];
    // unsigned int  rd_en_calib_total;
    ...
    one_shmoo_RE_FIFO( word_new_step, byte_new_step, option, phy_reg_base, memc_reg_base, mem_test_base );

    for ( wl = 0 ; wl < (((option >> 8) & 0xFF) / BITS_PER_WL) ; wl ++ ) {
        memsys_reg_offset = wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + GLOBAL_REG_RBUS_START;

        // write back BYTE_RD_EN values: 2 new values, one per WL
        data = 0;
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_en , 1 );
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_force , 1 );
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_step , word_new_step[ wl ] );
        ...
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , byte_sel , 1 );
#endif
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , ovr_step, byte_new_step[ wl * BYTE_BIT_BASE ] );
        tb_w( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + memsys_reg_offset , data );

        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , ovr_step , byte_new_step[ wl * BYTE_BIT_BASE + 1 ] );
        tb_w( DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + memsys_reg_offset, data );
    }
...
*   If all 64 result INT are stack in a column, the shmoo plot matches the 1 & 0 of the data if they rotate by -90 degrees
...
*   For a BIT plot, the X is driven individually - and therefore required an array size of 16 (instead of 2)
...
*          result[0]         result[63]
*/  
static void plot_result( unsigned int *result , unsigned int *step , unsigned int bit_count , int marker2 )
{
    unsigned int i;
    unsigned int bit;
    char str[ 65 ];
    ...
    NL();  NL();
    UART_OUT( SPACE );  UART_OUT( SPACE );  UART_OUT( SPACE ); UART_OUT( SPACE );
    for ( i = 0; i < 64; i++ ) UART_OUT('0' + ( i/10 ));
    NL();
    UART_OUT(SPACE);  UART_OUT(SPACE);  UART_OUT(SPACE);UART_OUT(SPACE);
    for ( i = 0; i < 64; i++ ) UART_OUT( '0' + ( i%10 ));
    NL();
    ...
    for ( bit = 0; bit < bit_count; bit++ ) {
        if (bit <= 9){ UART_OUT (SPACE); UART_OUT(ZERO); UART_OUT(bit + ZERO);}
        else { UART_OUT (SPACE); UART_OUT (ONE); UART_OUT(ZERO+(bit-10));}
        UART_OUT (SPACE);
        for ( i = 0; i < 64; i++ ) {
            if ( i == step[bit] ) str[i] = 'X';
            else if (i == (unsigned int)marker2)  str[i] = 'S';
            else                                  str[i] = ( result[i] & ( 1 << bit ) )? '-' : '+';
            UART_OUT(str[i]);
        } // for i
        NL();
    } // for bit
                                 ...
                                 unsigned long phy_reg_base)
{
    ...
    */
    ...
    for ( bit = 0 , marker = 1 , all_passed = FALSE ; bit < BITS_PER_WL ; bit++ , marker = marker << 1 ) {
        pass_cnt = 0 , fail_cnt = 0 , last_stat = 0xff;
        going_up = 0xff; going_down = 0xff;
        up = 0, down = -1; win_size_last = 0, win_size = 0;
        ...
        for ( vdl = 0 , pass_found = FALSE , fail_so_far = 0 ; vdl < VDL_STEPS ; vdl++ ) {
            ...
            if ( (last_stat != PASS_BIT) && (now_stat == PASS_BIT) && (pass_cnt == 0) ) {
                pass_cnt ++;
                last_pass_pos = vdl;
                //uart_out('+');plot_hex_number(last_pass_pos);
            }
            // detect passing edge
        ...
        if ( all_passed == FALSE ) { 
            ...
            ticks_per_clock = GET_FIELD( data , DDR40_CORE_PHY_CONTROL_REGS , VDL_CALIB_STATUS , calib_total );
            ...
            if ( going_up == 0xff ) {
                fail_vdl -= 1 ; // we need the position of the last pass just before the fail window
            ...
            // JIRA-14 error checking - make sure the VDL we choose is not a failing one
            // if so, pick the fail-pass transition
            if ( (result[ new_step[ bit ] ] & marker ) != PASS_BIT ) {
                new_step[bit] = last_pass_pos;
            }
        } 
        // case 3: all pass - take half of vdl from pass point to the end as the adjustment
        else { 
                                 ...
                                 unsigned long memc_reg_base , unsigned long mtest_addr , unsigned int mt_mode )
{
    ...
    for ( setting = 0 ; setting < 64 ; setting++ ) { // for each step
        ...
        timeout_ns( TIMEOUT_VAL );
        m = setting & 0x7;
        result[ setting ] = mtest_verify( memc_reg_base , mtest_addr , data_expected , mt_mode , wl );
    } // for setting

    return retval;
}

unsigned int generic_shmoo_calib_wr_dq( unsigned long phy_reg_base,
                                        unsigned long memc_reg_base,
                                        unsigned int option,
                                        unsigned long mem_test_base )
{
    ...
    for ( wl = 0 ; wl < (((option >> 8) & 0xFF) / BITS_PER_WL) ; wl++ ) {
        ...
        wr_calib_total = GET_FIELD( data, DDR40_CORE_PHY_CONTROL_REGS , VDL_WR_CHAN_CALIB_STATUS , wr_chan_calib_total ) >> 4;
        ...
        for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) {
            ...
            for ( i = 0; i < 8; i++ ) {
                data = RDREG( a + (i * 4) );
                en   = GET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT0_W , ovr_en );
                step = GET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT0_W , ovr_step );
...
static unsigned generic_shmoo_rd_dqs( int wl, unsigned int * result, int start_step, unsigned long phy_reg_base, 
                                      unsigned long memc_reg_base, unsigned long mtest_addr, unsigned int mt_mode );
static unsigned generic_shmoo_rd_dqs( int wl, unsigned int * result, int start_step, unsigned long phy_reg_base, 
                                      unsigned long memc_reg_base, unsigned long mtest_addr, unsigned int mt_mode )
{
    ...
    for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) {
        ...
        rd_en_setting[ bl ] = GET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , ovr_step );
    } // for bl
    ...
    data = RDREG( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_DQ_CALIB_STATUS ); // DQCS_PCR
    dqs_setting = GET_FIELD( data, DDR40_CORE_PHY_CONTROL_REGS , VDL_DQ_CALIB_STATUS, dqs_calib_total ) >> 4;

    all_fail = 0;
    while ( all_fail == 0 ) {
        timeout_ns( TIMEOUT_VAL );
        result[ dq_setting ] = mtest_verify( memc_reg_base , mtest_addr , data_expected , mt_mode , wl );
        if ( result[ dq_setting ] == 0xFFFF ) {
            ...
            data |= ( 1 << 8 ); // byte_sel = 1;
            WRREG( (wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_R_P)     , data ); // byte0 RP
            WRREG( (wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_R_P + 4) , data ); // byte0 RN
            WRREG( (wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_R_P )    , data ); // byte1 RP JIRA-11
            WRREG( (wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_R_P + 4) , data ); // byte1 RN JIRA-11
            for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) { // for each byte
                ...
                // write rd_en_setting to PHY WL{0,1} VDL_OVRIDE_BYTE{0,1}_BIT_RD_EN reg
                data = SET_OVR_STEP( rd_en_setting[ bl ] );
                // Start TODD 200911 need byte_sel
                data |= ( 1 << 8 ); // byte_sel = 1;
                // Finish TODD 200911 need byte_sel
                WRREG( wl * PHY_REG_OFFSET_PER_WL + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + 
    ...
    for ( bit = 0 ; bit < 16 ; ++bit ) { // for each bit
        ...
        while ( start == 1 ) {
            for ( inx = 0 , bit_result_sum = 0 ; inx < MAX_BIT_TO_CHECK ; inx++ ) {
                // check the next 6 bit results (a fail is a 1) expect all results to be 0 for a pass
                bit_result_sum += result[ i + inx ] & (1 << bit);
            } 
            // bit_result_sum = 0 if all bit results are pass, otherwise it is non-zero
            if ( bit_result_sum == 0) {
                pass_start[ bit ] = i;
                start = 0;
            }
        ...
        while ( finish == 1 ) {
            ...
            for ( inx = 0 , bit_total_failed = 1 ; inx < FAIL_BIT_TO_CHECK ; inx ++ ) {
                if ((result[ i + inx ] & (1 << bit)) == 0) {
                    // if any one of the result is a pass, then we are not there yet
                    bit_total_failed = 0;
                    break;
                }
            }
...
static int wr_dqs_setting( int wl , unsigned * result , int *init_step , int *new_step , 
                           unsigned long phy_reg_base , unsigned long memc_reg_base , 
                           unsigned long mtest_addr , int start_point , int which );
static int wr_dqs_setting( int wl , unsigned * result , int *init_step , int *new_step , 
                            unsigned long phy_reg_base , unsigned long memc_reg_base , 
                            unsigned long mtest_addr , int start_point , int which ) 
{
    ...
    for ( dq_setting = 0 ; dq_setting < 64 ; dq_setting++ ) { // for each setting
        ...
        data = SET_OVR_STEP( dq_setting );
        for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) { // for each byte
            byte_bit_rp_addr = (wl * PHY_REG_OFFSET_PER_WL) + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT0_R_P + 
                               PHY_REG_OFFSET_PER_BL * bl;
            byte_bit_rn_addr = (wl * PHY_REG_OFFSET_PER_WL) + phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT0_R_N + 
                               PHY_REG_OFFSET_PER_BL * bl;
            for ( bit = 0; bit < 8; bit++ ) {
                if ( which & 1 ) {
                    WRREG( byte_bit_rp_addr + (bit * 8) , data );
    ...
    for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) {       
       ...
       for ( i = start; i < 16; i += delta ) { // 8 R_Ps and 8 R_Ns
           ...
           // printf( "new_step[ %d ] ===> Reg[ 0x%X ]\n" , n , byte_bit_rp_addr + (i * 4) - (DDR_PHY_CONTROL_REGS_REVISION) );
           data = SET_OVR_STEP( new_step[ n ] );
                                     ...
                                     unsigned long mem_test_base )
{
    ...
    for ( wl = 0 ; wl < (((option >> 8) & 0xFF) / BITS_PER_WL) ; wl++ ) {
        ...
        retval |= wr_dqs_setting( wl , result , init_step , new_step , phy_reg_base , memc_reg_base , mem_test_base , start_step , 3 /* P + N */ );
        ...
        //
        retval |= wr_dqs_setting( wl , result , init_step , new_step , phy_reg_base , memc_reg_base , mem_test_base , start_step , 1 /* P */  );
        ...
        //
        retval |= wr_dqs_setting( wl , result , init_step , new_step , phy_reg_base , memc_reg_base , mem_test_base , start_step , 2 /* N */ ); 
                        ...
                        unsigned int mt_mode , unsigned int at_size , unsigned int wrd_ln , unsigned int high_stress )
{
    ...
    if      (high_stress==3) size = 16 * 1024  * 1024; 
    else if (high_stress==2) size =  8 * 1024  * 1024; 
    else if (high_stress==1) size =  1 * 1024  * 1024; 
    else size = 2*1024;
...
unsigned generic_shmoo_calib_addr( unsigned long phy_reg_base,
                                   unsigned long memc_reg_base,
                                   unsigned int option,
                                   unsigned long mem_test_base )
{
    ...
    //    unsigned orig_fullness;
    //    unsigned orig_cmdsize;
    ...
    calib_vdl_step = calib_vdl_step & DDR40_CORE_PHY_CONTROL_REGS_VDL_WR_CHAN_CALIB_STATUS_wr_chan_calib_total_MASK;
    calib_vdl_step = calib_vdl_step >> (DDR40_CORE_PHY_CONTROL_REGS_VDL_WR_CHAN_CALIB_STATUS_wr_chan_calib_total_SHIFT + 4);
    ...
    // added to fix a hang at code copy that looks like a mode reg issue - these are in the Toronto address shmoo code - Todd
    if ( run_addx_shmoo ) {
        ...
        for ( wl = 0; (unsigned int)wl < wl_count; wl++ ) {
            ...
            for ( i = start_vdl_val - 1 ; i >= 0 ; i-- ) {
                // Clear the FIFO status for this WL
                tb_w( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + memsys_reg_offset , 1 );
                // tb_w( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + memsys_reg_offset , 0 );      
                if ( wl ) {
                    tb_w( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + memsys_reg_offset1 , 0x1 );
                    // tb_w( DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + memsys_reg_offset1, 0x0);
                }    

                WRREG( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL , (0x10000 | i) ); // ovr_en = 1, ovr_step = *                  

                if ( !run_addr_test( memc_reg_base , mem_test_base , data , wl_count , (option >> 16)& 0xFF ,   // Log2( DDR SIZE ) -1  ( i.e. 256MB => 27 )
                                     wl , high_stress ) 
                   )
                {
                    pass_start = i;
        ...
        if ( pass_end > pass_start ) {
            ...
            WRREG( phy_reg_base + DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL , (0x10000 | new_vdl_step) ); // ovr_en = 1, ovr_step = *
        }
        else {
            new_vdl_step = 0;
...
// 
static inline unsigned one_shmoo_rd_data_dly_FIFO( unsigned int * new_step , ddr40_addr_t phy_reg_base , 
                                                     ddr40_addr_t memc_reg_base , unsigned int wl_count ,
                                                   ddr40_addr_t  mem_test_base );
static inline unsigned one_shmoo_rd_data_dly_FIFO( unsigned int * new_step , ddr40_addr_t phy_reg_base , 
                                                   ddr40_addr_t memc_reg_base , unsigned int wl_count ,
                                                   ddr40_addr_t  mem_test_base )
{
    ...
    //
    // initialize with 0 for the wl_count > 1. Can be moved to the else clause, see the following statement:
    // if ( wl_count > 1 ) {
    //
    ...
    regval1 = tb_r( phy_reg_base + DDR40_PHY_WORD_LANE_DRIVE_PAD_CTL_OFFSET + GLOBAL_REG_RBUS_START + memsys_reg_offset ); 
    SET_FIELD( regval1 , DDR40_PHY_WORD_LANE , DRIVE_PAD_CTL , DQS_ALWAYS_ON , 0x0 );
    tb_w( phy_reg_base + DDR40_PHY_WORD_LANE_DRIVE_PAD_CTL_OFFSET + GLOBAL_REG_RBUS_START + memsys_reg_offset, regval1 );
    tb_w( phy_reg_base + DDR40_PHY_WORD_LANE_DRIVE_PAD_CTL_OFFSET + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START + memsys_reg_offset , regval1 );
#endif
    ...
    original_RD_EN_BYTE_setting[ 0 ] = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + GLOBAL_REG_RBUS_START );
    original_RD_EN_setting[ 0 ]      = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + GLOBAL_REG_RBUS_START );
    original_RD_EN_setting[ 1 ]      = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + GLOBAL_REG_RBUS_START );
    if ( wl_count > 1 ) {
        original_RD_EN_BYTE_setting[ 1 ] = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START );
        original_RD_EN_setting[ 2 ] = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START);
        original_RD_EN_setting[ 3 ] = tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START);
    }
    ...
    SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_en , 1 );
    SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_force , 1 );  
    SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE_RD_EN , ovr_step , RD_EN_word_setting );

    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + GLOBAL_REG_RBUS_START , data );
    if ( wl_count > 1 ) { 
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , data );
    }
    ...
    SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , byte_sel , 1 );
#endif
    SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , VDL_OVRIDE_BYTE0_BIT_RD_EN , ovr_step , RD_EN_byte_setting );
    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + GLOBAL_REG_RBUS_START , data );
    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + GLOBAL_REG_RBUS_START , data);
    if ( wl_count > 1 ) {
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , data );
    ...
    while ( passing == 0x0 ) {
        ...
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_DATA_DLY + GLOBAL_REG_RBUS_START , data );
        if ( wl_count > 1 ) {
            tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_DATA_DLY + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , data );
        }
        ...
        fifo_result[ 0 ] = GET_FIELD( tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + GLOBAL_REG_RBUS_START ) , 
                                      DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status0 );
        fifo_result[ 1 ] = GET_FIELD( tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + GLOBAL_REG_RBUS_START ) , 
                                      DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status1 );
        if ( wl_count  > 1) {
            nTest_1 = mtest_verify ( memc_reg_base , mem_test_base , data_expected , 0 , 1 );

            fifo_result[ 2 ] = GET_FIELD( tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + 
                                                PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START ) ,
                                          DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status0 );
            fifo_result[ 3 ] = GET_FIELD( tb_r( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_STATUS + 
                                                PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START ) ,
                                          DDR40_CORE_PHY_WORD_LANE_0 , READ_FIFO_STATUS , status1 );
        }
        // Clear the FIFO status
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + GLOBAL_REG_RBUS_START , 0x1 );
        // tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + GLOBAL_REG_RBUS_START , 0x0 );
        if ( wl_count > 1 ) {
            tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , 0x1 );
            // tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_READ_FIFO_CLEAR + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , 0x0 );
        }
        ...
        if ( (wl0_bl0_pass > 0x0) && (wl0_bl1_pass > 0x0) && (wl1_bl0_pass > 0x0) && (wl1_bl1_pass > 0x0) ) {
            passing = 1;
        }
        else {
        }
    ...
    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + GLOBAL_REG_RBUS_START , original_RD_EN_BYTE_setting[ 0 ] );
    if ( wl_count  > 1 ) {
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , original_RD_EN_BYTE_setting[ 1 ] );
    }

    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + GLOBAL_REG_RBUS_START , original_RD_EN_setting[ 0 ] );
    tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + GLOBAL_REG_RBUS_START , original_RD_EN_setting[ 1 ] );
    if ( wl_count > 1 ) {
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , original_RD_EN_setting[ 2 ] );
        tb_w( phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + PHY_REG_OFFSET_PER_WL + GLOBAL_REG_RBUS_START , original_RD_EN_setting[ 3 ] );                               
    }

    return retval;
}

unsigned all_shmoo_rd_data_dly_FIFO( ddr40_addr_t phy_reg_base,
                                     ddr40_addr_t memc_reg_base,
                                     unsigned int option,
                                     ddr40_addr_t mem_test_base )
{
    ...
    for ( wl = 0 ; wl < wc_count ; wl ++ ) {
        pack328_t packname;
        unsigned long memsys_reg_offset =  wl * PHY_REG_OFFSET_PER_WL + phy_reg_base +  GLOBAL_REG_RBUS_START;
        ...
        SET_FIELD( data , DDR40_CORE_PHY_WORD_LANE_0 , READ_DATA_DLY , rd_data_dly , new_step[ wl ] );
...
unsigned int generic_shmoo_calib_wr_dm( unsigned long phy_reg_base,
                                        unsigned long memc_reg_base,
                                        unsigned int option,
                                        unsigned long mem_test_base )
{
    ...
    for ( wl = 0 ; wl < (((option >> 8) & 0xFF) / BITS_PER_WL) /*K_PHY_CFG__WC_COUNT*/; wl ++ ) {
        for ( bl = 0 ; bl < BL_PER_WL ; bl++ ) {
            step_inx = (wl * 2) + (bl * 1);
            ...
            for ( bit = 0 ; bit < BITS_PER_BL ; bit ++ ) {
                wrdq_val[ bit ] = (RDREG(val + (bit * 4))) & DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT0_W_ovr_step_MASK /*WRDQ_MASK*/;
            ...
            WRREG( (wl * PHY_REG_OFFSET_PER_WL + PHY_REG_OFFSET_PER_BL * bl + 
                   phy_reg_base + DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_DM_W) , data );
        }
        NL();
    }
    NL ();
...
//   -3   shift too much.  
//    0   OK
int shmoo_adjust_addx_ctrl_delay( unsigned long phy_reg_base , unsigned long memc_reg_base, int shift_amount ) 
{
    ...
    //  uart_out('C');uart_out('A');uart_out('L');uart_out('I');uart_out('B');
    //  uart_out('S');uart_out('T');uart_out('S');uart_out(' ');

    regdata = 0;
    II = 0;

    regdata = tb_r( phy_reg_base + GLOBAL_REG_RBUS_START + DDR40_CORE_PHY_CONTROL_REGS_VDL_WR_CHAN_CALIB_STATUS );

    calib_bit_offset = GET_FIELD( regdata , DDR40_CORE_PHY_CONTROL_REGS , VDL_WR_CHAN_CALIB_STATUS , wr_chan_calib_bit_offset );
    wr_chan_calib_total = GET_FIELD( regdata , DDR40_CORE_PHY_CONTROL_REGS , VDL_WR_CHAN_CALIB_STATUS , wr_chan_calib_total );
    ...
    // BYTE MODE
    if ( DDR40_CORE_PHY_CONTROL_REGS_VDL_WR_CHAN_CALIB_STATUS_wr_chan_calib_byte_sel_MASK & regdata ) {
        return ERR_AC_VDL_SHIFT_BYTE_MODE_NOT_SUPPORTED;
    } 
    // BIT MODE
    else {
        steps = ( WR_VDL_FIXED_DLY + (wr_chan_calib_total >> 4) ) ;    // WR_VDL_FIXED_DLY (about 8, comes from the VDL fixed delay.)
        ...
        //      uart_out('+'); plot_dec_number(incr) ; uart_out('=');
        //      plot_dec_number(steps); uart_out('*');plot_dec_number(shift_amount);
        //      uart_out('/') ; uart_out('1'); uart_out('6');   NL();

        if ( (incr + steps) > 63 ) {
            bytedata  = DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BYTE_CTL_ovr_en_MASK |
                        ( ( ((incr + steps) >> 1) - WR_VDL_FIXED_DLY) & DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BYTE_CTL_ovr_step_MASK ); 
            //        Set VDL_OVRIDE_BYTE_CTL.byte_sel = 0 ;
            //        Set VDL_OVRIDE_BYTE_CTL.ovr_step = (((incr + steps) >> 1) - WR_VDL_FIXED_DLY) ;    // This -8 covers the additional fixed delays.
            ...
            bitdata = DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL_ovr_en_MASK |
                      ( (incr + steps) & DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL_ovr_step_MASK );
            //        Set VDL_OVRIDE_BIT_CTL.byte_sel = 0 ;
            //        Set VDL_OVRIDE_BIT_CTL.ovr_step = (incr + steps);
        }

        // update reg
        tb_w( phy_reg_base + GLOBAL_REG_RBUS_START + DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BYTE_CTL , bytedata );
        tb_w( phy_reg_base + GLOBAL_REG_RBUS_START + DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL , bitdata );
    }
