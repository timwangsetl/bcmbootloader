...
#include <asm/iproc/ddr40_phy_registers.h>
...
//	  SET_FIELD(data, DDR40_CORE_PHY_CONTROL_REGS, PLL_DIVIDERS, POST_DIV, 1); 
    ...
    while((timeout>0) && ((tmp & DDR40_CORE_PHY_CONTROL_REGS_PLL_STATUS_LOCK_MASK) == 0)){
...
FUNC_PREFIX void ddr40_phy_addr_ctl_adjust(uint32_t total_steps, ddr40_addr_t offset) FUNC_SUFFIX
{
    ...
    if (GET_FIELD(data, DDR40_CORE_PHY_CONTROL_REGS, VDL_WR_CHAN_CALIB_STATUS, wr_chan_calib_byte_sel) == 0) {
        // we don't do adjustment if we are in BYTE mode, because it means the clock is very slow and no adjustment is needed
        tmp = 0;
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_OVRIDE_BIT_CTL, ovr_step,
                  GET_FIELD(data, DDR40_CORE_PHY_CONTROL_REGS, VDL_WR_CHAN_CALIB_STATUS, wr_chan_calib_total) >> 4);
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_OVRIDE_BIT_CTL, ovr_en, 1);
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_OVRIDE_BIT_CTL, byte_sel, 1); // use BYTE VDLs for adjustment, so switch to BYTE mode
        
        DDR40_PHY_RegWr( DDR40_CORE_PHY_CONTROL_REGS_VDL_OVRIDE_BIT_CTL + offset, tmp);

        tmp = 0;
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_OVRIDE_BYTE_CTL, ovr_step, (total_steps > 10) ? (total_steps - 10)/2 : 0); // avoid negative
...
/* void ddr40_phy_rd_en_adjust(uint32_t total_steps0, uint32_t total_steps1, uint32_t rd_en_byte_mode, ddr40_addr_t offset, uint32_t wl_offset) */
...
FUNC_PREFIX void ddr40_phy_rd_en_adjust(uint32_t total_steps0, uint32_t total_steps1, uint32_t rd_en_byte_mode, ddr40_addr_t offset, uint32_t wl_offset) FUNC_SUFFIX
{
    uint32_t tmp, bit_vdl_steps0, bit_vdl_steps1, byte_vdl_steps, fixed_steps, adj_steps0, adj_steps1 ;
    ...
    if (byte_vdl_steps > 63) byte_vdl_steps = 63;
    ...
    SET_FIELD(tmp, DDR40_CORE_PHY_WORD_LANE_0, VDL_OVRIDE_BYTE_RD_EN, ovr_step, byte_vdl_steps);
    DDR40_PHY_RegWr((DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE_RD_EN + offset + wl_offset), tmp);
    ...
    SET_FIELD(tmp, DDR40_CORE_PHY_WORD_LANE_0, VDL_OVRIDE_BYTE0_BIT_RD_EN, ovr_step, bit_vdl_steps0);
    DDR40_PHY_RegWr((DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_BIT_RD_EN + offset + wl_offset), tmp);
    ...
    SET_FIELD(tmp, DDR40_CORE_PHY_WORD_LANE_0, VDL_OVRIDE_BYTE1_BIT_RD_EN, ovr_step, bit_vdl_steps1);
    DDR40_PHY_RegWr((DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE1_BIT_RD_EN + offset + wl_offset), tmp);
    DDR40_PHY_Print("ddr40_phy_init:: VDL_OVRIDE_BYTEx_BIT_RD_EN set to: 0x%02X (%d)\n",tmp, tmp & 0x3F);
}

#ifdef DDR40_INCLUDE_ECC
/* void ddr40_phy_ecc_rd_en_adjust(uint32_t total_steps0, uint32_t rd_en_byte_mode, ddr40_addr_t offset) */
/*     total_steps0 - Desired delay for byte lane 0 within the word lane in steps */
/*     rd_en_byte_mode - Byte (pair) mode vs. bit (single) mode for RD_EN VDL */
/*     offset - Address beginning of PHY register space in the chip register space */
FUNC_PREFIX void ddr40_phy_ecc_rd_en_adjust(uint32_t total_steps0, uint32_t rd_en_byte_mode, ddr40_addr_t offset) FUNC_SUFFIX
{
    ...
    SET_FIELD(tmp, DDR40_CORE_PHY_ECC_LANE, VDL_OVRIDE_BYTE_BIT_RD_EN, ovr_step, bit_vdl_steps0);
...
FUNC_PREFIX uint32_t ddr40_phy_vdl_normal(uint32_t vdl_no_lock, ddr40_addr_t offset) FUNC_SUFFIX
{
    ...
    //SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIBRATE, calib_clocks, 0); // 0 - 1 clock
    ...
    while((timeout>0) && (GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIB_STATUS, calib_idle) == 0)) {
        DDR40_PHY_Timeout(1000);
        timeout--;
        tmp = DDR40_PHY_RegRd(DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS + offset);
    }
    ...
    if (GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIB_STATUS, calib_lock) == 0) {
        ...
        if (vdl_no_lock)
            DDR40_PHY_Print("DDR PHY VDL calibration complete but did not lock! Step = %d\n", 
                        GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIB_STATUS, calib_total) >> 4);
        else {
            DDR40_PHY_Error("DDR PHY VDL calibration complete but did not lock! Step = %d\n", 
                        GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIB_STATUS, calib_total) >> 4);
...
/*     offset - Address beginning of PHY register space in the chip register space */
FUNC_PREFIX void ddr40_phy_vtt_on(uint32_t connect, uint32_t override, ddr40_addr_t offset) FUNC_SUFFIX
{
    uint32_t tmp;
...
FUNC_PREFIX uint32_t ddr40_phy_calib_zq(uint32_t params, ddr40_addr_t offset) FUNC_SUFFIX
{
    ...
    if (params & DDR40_PHY_PARAM_MAX_ZQ) {
        tmp = 0;
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, ZQ_PVT_COMP_CTL, dq_nd_override_val, 7);
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, ZQ_PVT_COMP_CTL, dq_pd_override_val, 7);
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, ZQ_PVT_COMP_CTL, addr_nd_override_val, 7);
        SET_FIELD(tmp, DDR40_CORE_PHY_CONTROL_REGS, ZQ_PVT_COMP_CTL, addr_pd_override_val, 7);
    ...
    while((timeout>0) && (GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, ZQ_PVT_COMP_CTL, sample_done) == 0)) {
      DDR40_PHY_Timeout(1000);
      timeout--;
      tmp = DDR40_PHY_RegRd(DDR40_CORE_PHY_CONTROL_REGS_ZQ_PVT_COMP_CTL + offset);
    }
    ...
    if(timeout<=0) {
      DDR40_PHY_Fatal("ddr40_phy_init.c: ddr40_phy_init: DDR PHY ZQ Calibration failed\n");
      return (DDR40_PHY_RETURN_ZQ_CALIB_FAIL);
    }
    else
      DDR40_PHY_Print("ddr40_phy_init: ZQ Cal complete.  ND_COMP = %d, PD_COMP = %d, ND_DONE = %d, PD_DONE = %d\n",
...
/*     params - Set of flags */
/*     offset - Address beginning of PHY register space in the chip register space */
FUNC_PREFIX void ddr40_phy_rdly_odt(uint32_t speed, uint32_t params, ddr40_addr_t offset) FUNC_SUFFIX
{
    uint32_t data, dly;
    ...
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_0, READ_CONTROL, dq_odt_enable, (params & (DDR40_PHY_PARAM_DIS_DQS_ODT | DDR40_PHY_PARAM_DIS_ODT)) ? 0:1);
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_0, READ_CONTROL, dq_odt_le_adj, (params & DDR40_PHY_PARAM_ODT_EARLY) ? 1 : 0);
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_0, READ_CONTROL, dq_odt_te_adj, (params & DDR40_PHY_PARAM_ODT_LATE) ? 1 : 0);
...
/*     params - Set of flags */
/*     offset - Address beginning of PHY register space in the chip register space */
FUNC_PREFIX void ddr40_phy_ddr3_misc(uint32_t speed, uint32_t params, ddr40_addr_t offset) FUNC_SUFFIX
{
    uint32_t data;
    uint32_t vddo_volts;

    vddo_volts = ((params & DDR40_PHY_PARAM_VDDO_VOLT_0)? 1: 0) + ((params & DDR40_PHY_PARAM_VDDO_VOLT_1)? 2: 0);
...
/*     params - Set of flags */
/*     offset - Address beginning of PHY register space in the chip register space */
FUNC_PREFIX void ddr40_phy_ddr2_misc(uint32_t speed, uint32_t params, ddr40_addr_t offset) FUNC_SUFFIX
{
    uint32_t vddo_volts;
    uint32_t data;
    ...
    data = (params & DDR40_PHY_PARAM_LONG_PREAMBLE) ? DDR40_CORE_PHY_WORD_LANE_0_WR_PREAMBLE_MODE_long_MASK : 0; // DDR2
...
FUNC_PREFIX void ddr40_phy_set_autoidle(uint32_t params, ddr40_addr_t offset) FUNC_SUFFIX
{
    ...
    iddq_code = ((params & DDR40_PHY_PARAM_AUTO_IDDQ_VALID)? 1:0) + ((params & DDR40_PHY_PARAM_AUTO_IDDQ_CMD)? 2:0);
    rxenb_code = ((params & DDR40_PHY_PARAM_AUTO_RXENB_VALID)? 1:0) + ((params & DDR40_PHY_PARAM_AUTO_RXENB_CMD)? 2:0);

    data = DDR40_PHY_RegRd ( DDR40_CORE_PHY_WORD_LANE_0_IDLE_PAD_CONTROL + offset);
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_0, IDLE_PAD_CONTROL, auto_dq_iddq_mode, iddq_code);
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_0, IDLE_PAD_CONTROL, auto_dq_rxenb_mode, rxenb_code);
    ...
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_1, IDLE_PAD_CONTROL, auto_dq_iddq_mode, iddq_code);
    SET_FIELD(data, DDR40_CORE_PHY_WORD_LANE_1, IDLE_PAD_CONTROL, auto_dq_rxenb_mode, rxenb_code);
    ...
    SET_FIELD(data, DDR40_CORE_PHY_ECC_LANE, IDLE_PAD_CONTROL, auto_dq_iddq_mode, iddq_code);
    SET_FIELD(data, DDR40_CORE_PHY_ECC_LANE, IDLE_PAD_CONTROL, auto_dq_rxenb_mode, rxenb_code);
...
/*     connect - Mask of the signals connected to the Virtual VTT capacitor (see RDB for specific bits) */
/*     override - Mask of the signals used to control voltage on the Virtual VTT capacitor (see RDB for specific bits) */
/*     offset - Address of beginning of PHY register space in the chip register space */
...
FUNC_PREFIX unsigned int ddr40_phy_init(uint32_t ddr_clk, uint32_t params, int ddr_type, uint32_t * wire_dly, uint32_t connect, uint32_t override, ddr40_addr_t offset) FUNC_SUFFIX
{
  /* This coding style does not work in MIPS boot loader, as it requires that the memory that
   * holds the offset be initialized before the program starts.  This data segment runs out of SID SRAM
   * and must not be initialize by the compiler, as this would force a pre-load of memory
   * Look at the alternative coding style in this file.

  *************************************************/
  uint32_t tmp, ddr_clk_per, cal_steps, step_size, timeout ;
  uint32_t /*rd_en_byte_mode, rd_en_byte_vdl_steps, rd_en_bit_vdl_offset,*/ total_steps0/*, total_steps1 */;
  uint32_t tmode;
  ...
  if (! tmode) {
      ...
      return_code |= ddr40_phy_vdl_normal((params & DDR40_PHY_PARAM_ALLOW_VDL_NO_LOCK)? 1: 0, offset);
      ...
      while((timeout>0) && ((tmp & DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS_calib_idle_MASK) == 0)){
          DDR40_PHY_Timeout(1000);
          tmp = DDR40_PHY_RegRd(DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS + offset);
          timeout--;
      }

      if ((timeout <= 0) || ((tmp & DDR40_CORE_PHY_CONTROL_REGS_VDL_CALIB_STATUS_calib_lock_MASK) == 0)) {
          DDR40_PHY_Error("ddr40_phy_init.c: DDR PHY step size calibration failed.\n");
          step_size = DDR40_PHY_DEFAULT_STEP_SIZE ;
          cal_steps = ddr_clk_per/step_size;
          return_code |= DDR40_PHY_RETURN_STEP_CALIB_FAIL;
      } else {
          DDR40_PHY_Print("ddr40_phy_init::DDR PHY step size calibration complete.\n");
          cal_steps = GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_CALIB_STATUS, calib_total) ;
          step_size = (ddr_clk_per / cal_steps) ;
      }
      
      tmp = ((params & DDR40_PHY_PARAM_ADDR_CTL_ADJUST_1) ? 2 : 0) + ((params & DDR40_PHY_PARAM_ADDR_CTL_ADJUST_0) ? 1 : 0);
      ...
      if ( ! (params & DDR40_PHY_PARAM_SKIP_RD_EN_ADJUST)) {
          ...
          // The READ enable VDL calibration tracks the DQS setting (half bit period or full bit period).
          
          tmp = DDR40_PHY_RegRd(DDR40_CORE_PHY_CONTROL_REGS_VDL_RD_EN_CALIB_STATUS + offset);
          rd_en_byte_mode = GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_RD_EN_CALIB_STATUS, rd_en_calib_byte_sel) ;
          rd_en_byte_vdl_steps = GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_RD_EN_CALIB_STATUS, rd_en_calib_total) >> 4;
          rd_en_bit_vdl_offset = GET_FIELD( tmp, DDR40_CORE_PHY_CONTROL_REGS, VDL_RD_EN_CALIB_STATUS, rd_en_calib_bit_offset) ;
          ...
          DDR40_PHY_Print("ddr40_phy_init:: Calibrated Read Enable VDL Steps =  0x%X (%d)\n", tmp, tmp);

          total_steps0 = ((rd_en_byte_vdl_steps << rd_en_byte_mode) + (wire_dly[0] / step_size) + rd_en_bit_vdl_offset) ;
          total_steps1 = ((rd_en_byte_vdl_steps << rd_en_byte_mode) + (wire_dly[1] / step_size) + rd_en_bit_vdl_offset) ;

          DDR40_PHY_Print("ddr40_phy_init:: BL0 RD_EN adjustment (%d ps): Total Steps =  0x%X (%d)\n", wire_dly[0], total_steps0, total_steps0);
          DDR40_PHY_Print("ddr40_phy_init:: BL1 RD_EN adjustment (%d ps): Total Steps =  0x%X (%d)\n", wire_dly[1], total_steps1, total_steps1);

          ddr40_phy_rd_en_adjust (total_steps0, total_steps1, rd_en_byte_mode, offset, (DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_W - DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_W));

#ifdef DDR40_WIDTH_IS_32
          total_steps0 = ((rd_en_byte_vdl_steps << rd_en_byte_mode) + (wire_dly[2] / step_size) + rd_en_bit_vdl_offset) ;
          total_steps1 = ((rd_en_byte_vdl_steps << rd_en_byte_mode) + (wire_dly[3] / step_size) + rd_en_bit_vdl_offset) ;
          DDR40_PHY_Print("ddr40_phy_init:: BL2 RD_EN adjustment (%d ps): Total Steps =  0x%X (%d)\n", wire_dly[2], total_steps0, total_steps0);
          DDR40_PHY_Print("ddr40_phy_init:: BL3 RD_EN adjustment (%d ps): Total Steps =  0x%X (%d)\n", wire_dly[3], total_steps1, total_steps1);
          ddr40_phy_rd_en_adjust (total_steps0, total_steps1, rd_en_byte_mode, offset, (DDR40_CORE_PHY_WORD_LANE_1_VDL_OVRIDE_BYTE0_W - DDR40_CORE_PHY_WORD_LANE_0_VDL_OVRIDE_BYTE0_W));
#endif
#ifdef DDR40_INCLUDE_ECC
          total_steps0 = ((rd_en_byte_vdl_steps << rd_en_byte_mode) + (wire_dly[4] / step_size) + rd_en_bit_vdl_offset) ;
          DDR40_PHY_Print("ddr40_phy_init:: ECC RD_EN adjustment (%d ps): Total Steps =  0x%X (%d)\n", wire_dly[4], total_steps0, total_steps0);
  ...
  if ((ddr_type == 1)) {  // DDR3
      ...
      ddr40_phy_ddr2_misc(ddr_clk,params | DDR40_PHY_PARAM_ODT_EARLY,offset); // DDR-2 (type == 0) requires ODT_EARLY.

  ddr40_phy_set_autoidle(params, offset);

#ifdef SV_SIM
  DDR40_PHY_Timeout(400);  //required some delay (really 500 us) from PAD_CTL to CKE enable
#else
  DDR40_PHY_Timeout(500000);  //required some delay (really 500 us) from PAD_CTL to CKE enable
#endif
