...
//
// Previously it was BLOCK_SIZE and there was a conflict with the defines in the linux kernel header file
//
...
// void generic_calib_steps( ddr40_addr_t phy_control_reg_base );
// void generic_shmoo_cleanup( ddr40_addr_t phy_reg_base , unsigned int wl_in_use );
...
// Set this define to 1 to use TMT method to generate the data to read/write to memory during shmoo
// Set this define to 0 to use 0xAAAAAAAA and 0x55555555 patterns for the data
#define SHMOO_USE_TMT		1

// Set this define to 1 in order to use memory copy function (useful for Northstar platform)
...
#define SHMOO_VERSION ((PHY_DATA_WIDTH << PHY_DW_POS)|(PHY_WL_COUNT << PHY_WL_POS)|(SHMOO_VER_MAJ << SHMOO_VJ_POS)|SHMOO_VER_MIN_LONG) 
...
int shmoo_adjust_addx_ctrl_delay( ddr40_addr_t phy_reg_base , ddr40_addr_t memc_reg_base, int shift_amount );
...
void InitMemory( ddr40_addr_t const Address , unsigned long const nSizeDword , unsigned long const Seed );
...
void CleanMemory( ddr40_addr_t const Address , unsigned long const nLengthInDwords , unsigned int const nMask );
unsigned int CheckMemory( ddr40_addr_t const Address , unsigned long const nSizeDword , unsigned int const Seed , unsigned int const wrd_ln );
unsigned int CheckMemoryV2( ddr40_addr_t Address , unsigned long const nLengthDwords , unsigned int wrd_ln );
unsigned int CheckMemoryV3( ddr40_addr_t Address , unsigned long const nLengthDwords , unsigned int wrd_ln , int const j );
